...
> - ✅ **Phase 4 (signature visuelle)** : scrollytelling pinné « Du brut à l'œuvre » (`CraftStory`, GSAP scrub, 4 états du mur, monogramme LK tracé) · studio epoxy interactif (`EpoxyStudio`, 4 teintes temps réel + reflet clearcoat + devis prérempli) · **vidéos recompressées ffmpeg : ~14 Mo → ~5,9 Mo (−58 %)**.
> - ✅ **Phase 5 (début)** : tests honeypot (3 verts) · suite de tests backend verte · Dependabot (composer + npm web/mobile + actions).
> - ⏳ **Restant** : stockage R2/CDN (à provisionner côté Cloudflare) · version R3F/WebGL des scènes 3D si souhaitée plus tard · compteurs animés (en attente des vrais chiffres clés) · Sentry/uptime/2FA/audit-log (comptes et décisions à ta charge).
